...
# Set page config
st.set_page_config(
    page_title="D212 Task 3 - Market Basket Analysis",
    layout="wide",
...
if 'initialized' not in st.session_state:
    ...
    st.session_state.melted_df = None
    st.session_state.unique_meds = None
    st.session_state.diabetes_meds = None
    st.session_state.cardiovascular_meds = None
...
def load_data():
    try:
        ...
        data_path = Path("medical_market_basket.csv")
        
        # If not found, try the data subdirectory
        if not data_path.exists():
            data_path = Path("data/medical_market_basket.csv")
        
        # If still not found, try relative to script location
        if not data_path.exists():
            script_dir = Path(__file__).parent
            data_path = script_dir / "data" / "medical_market_basket.csv"
        
        if not data_path.exists():
            st.error(f"Data file not found. Tried paths: ./medical_market_basket.csv, ./data/medical_market_basket.csv, and {script_dir}/data/medical_market_basket.csv")
            st.stop()
